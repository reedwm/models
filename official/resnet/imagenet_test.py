...
class BaseTest(tf.test.TestCase):
  ...
  def resnet_model_fn_helper(self, mode, use_fp16):
  ...
  def test_resnet_model_fn_train_mode(self):
    self.resnet_model_fn_helper(tf.estimator.ModeKeys.TRAIN, use_fp16=False)
    self.resnet_model_fn_helper(tf.estimator.ModeKeys.TRAIN, use_fp16=True)

  def test_resnet_model_fn_eval_mode(self):
    self.resnet_model_fn_helper(tf.estimator.ModeKeys.EVAL, use_fp16=False)
    self.resnet_model_fn_helper(tf.estimator.ModeKeys.EVAL, use_fp16=True)

  def test_resnet_model_fn_predict_mode(self):
    self.resnet_model_fn_helper(tf.estimator.ModeKeys.PREDICT, use_fp16=False)
    self.resnet_model_fn_helper(tf.estimator.ModeKeys.PREDICT, use_fp16=True)
